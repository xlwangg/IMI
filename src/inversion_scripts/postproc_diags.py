...
def fill_missing_hour(run_name, run_dirs_pth, prev_run_pth, start_day):
    ...
    def process(r):
        ...
        # 0.125x0.3125 may set transport timestep as 150min
        if not os.path.exists(output_file_SC):
            output_file_SC = (f"{run_dirs_pth}/{r}/OutputDir/GEOSChem.SpeciesConc.{start_day}_0002z.nc4")            
        ...
        if "0000" in r or "background" in r:
            ...
            # 0.125x0.3125 may set transport timestep as 150min
            if not os.path.exists(output_file_LE):
                output_file_LE = f"{run_dirs_pth}/{r}/OutputDir/GEOSChem.LevelEdgeDiags.{start_day}_0002z.nc4"
...
def fill_missing_hour_posterior(run_dirs_pth, prev_run_pth, start_day):
    ...
    # 0.125x0.3125 may set transport timestep as 150min
    if not os.path.exists(output_file_SC):
        output_file_SC = (f"{run_dirs_pth}/OutputDir/GEOSChem.SpeciesConc.{start_day}_0002z.nc4")
    output_data_SC = xr.load_dataset(output_file_SC)
    output_file_LE = (
    ...
    # 0.125x0.3125 may set transport timestep as 150min
    if not os.path.exists(output_file_LE):
        output_file_LE = (f"{run_dirs_pth}/OutputDir/GEOSChem.LevelEdgeDiags.{start_day}_0002z.nc4")        
    output_data_LE = xr.load_dataset(output_file_LE)
