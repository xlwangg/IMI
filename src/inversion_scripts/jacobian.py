...
if __name__ == "__main__":
    ...
    lognormalErrors = sys.argv[12] #if len(sys.argv) > 12 else False
    ...
    if isPost.lower() == "false":  # if sampling prior simulation
        ...
        if lognormalErrors.lower() == "true":
            gc_cache = f"{workdir}/data_geoschem_posterior_ln"
            outputdir = f"{workdir}/data_converted_posterior_ln"
            vizdir = f"{workdir}/data_visualization_posterior_ln"
        else:      
    ...
    # for filename in sat_files:
        ...
        if not os.path.isfile(f"{outputdir}/{date}_GCtoTROPOMI.pkl"):
            ...
            if lognormalErrors.lower() == "true" and isPost.lower() == "false":
